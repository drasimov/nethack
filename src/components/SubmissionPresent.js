        ...
        <div className={`${styles.wrap} displayBox`}>
            {submission &&
                <div id={submission.teamID} className={styles.outline}>
                    <a target="_blank" href={override ? override:`/25R1/${submission.teamID}/${submission.title}.html`}><div className={styles.image}>
                        <img src={`/25R1/${submission.teamID}.jpg`}></img>
                        <div className={styles.play}>{iconPlay}</div>
                    </div></a>
                    <div className={styles.text}>
                        <p className = "cWhite medBig serifBold">{submission.title == null ? <span className="cRed">Untitled Submission</span> : submission.title}<span className="smallMed">&nbsp;</span></p>
                        <hr/>
